...
test("getUsageForProvider routes agy through the Antigravity usage implementation", async () => {
  ...
  try {
    const result = await usageModule.getUsageForProvider(
      {
        ...
        providerSpecificData: { projectId: "test-proj" },
      },
      { forceRefresh: true }
    );
    ...
    const quota = (result as { quotas: Record<string, any> }).quotas["gemini_5h"];
    assert.ok(quota, "should expose the upstream agy model family quota");
    assert.equal(quota.remainingPercentage, 75);
    assert.equal(
      (result as { quotas: Record<string, any> }).quotas["gemini-3.7-flash-high"],
      undefined,
      "agy quota should not expose individual per-model keys"
    );
  } finally {
...
test("parseQuotaData treats agy quota payloads like Antigravity", () => {
  ...
  assert.equal(
    parsed.length,
    2,
    "credits and model family quota should be rendered, models summary skipped"
  );
  ...
  const familyQuota = parsed.find((quota: any) => quota.name === "gemini_5h");
  assert.ok(familyQuota, "family quota should be rendered");
  assert.equal(familyQuota.remainingPercentage, 75);
});
